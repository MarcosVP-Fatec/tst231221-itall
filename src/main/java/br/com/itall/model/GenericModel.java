...
public abstract class GenericModel {
	 ...
	 * @throws InvocationTargetException Tratamento de erros de Runtime ao executar
	 *                                   o método.
	 * @throws IllegalArgumentException  Tratamento de erros de argumentos
	 *                                   inválidos.
	...
	/**
	 * Método que é executado uma vez para cada classe e que instancia os atributos
	 * padrões<br>
	 * do Model.
	 * 
	 * @author MarcosVP
		...
		for (Field campo : this.getClass().getDeclaredFields()) {
			...
			if (campo.isAnnotationPresent(CampoBD.class)) {
				...
				__fields.put(campo.getName(),
						new GenericDataAttribute(
								anotacao.field_name().isEmpty() ? campo.getName() : anotacao.field_name(),
								campo.getName(), anotacao.field_len(), anotacao.is_id(),
								anotacao.is_id() ? false : anotacao.is_to_insert(), anotacao.is_to_update()));
